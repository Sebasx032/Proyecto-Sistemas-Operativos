...
// Estructura para almacenar la información de cada hilo
typedef struct {
    int id;
    char *nombre_archivo;
    long lineas_leidas;
...
// -----------------------------------------------------------
// FUNCIÓN DE HILO: procesa un archivo contando líneas
// -----------------------------------------------------------
// Nota: Cada hilo opera sobre un archivo distinto, evitando
// dependencia entre hilos y permitiendo paralelismo real.
void *procesar_archivo(void *arg) {
    ...
    while (fgets(buffer, sizeof(buffer), f)) {
        contador++;
        // Simulación opcional de trabajo adicional
    ...
    // Zona crítica: solo se sincroniza lo necesario.
    // Esto reduce tiempo en mutex, mejorando el rendimiento.
...
int main() {
    ...
    for (int i = 0; i < NUM_HILOS; i++) {
        ...
        // Cada hilo recibe su estructura con parámetros independientes
